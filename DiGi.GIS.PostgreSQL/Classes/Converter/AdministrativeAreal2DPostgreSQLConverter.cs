...
namespace DiGi.GIS.PostgreSQL.Classes
{
    public class AdministrativeAreal2DPostgreSQLConverter : PostgreSQLConverter<AdministrativeAreal2D>
    {
        ...
        {
            List<AdministrativeAreal2D>? administrativeAreal2Ds = await GetAdministrativeAreal2DsByIdsAsync([id]);
            if (administrativeAreal2Ds is null || administrativeAreal2Ds.Count == 0)
            {
                return null;
            }
        ...
        public async Task<HashSet<string>?> GetCodesAsync()
        {
            ...
            string query = "SELECT code FROM administrative_areal_2D;";
        ...
        public async Task<HashSet<int>?> GetIdsAsync()
        {
            ...
            // We only select the 'id' column to minimize data transfer
            string query = "SELECT id FROM administrative_areal_2D;";
        ...
        public async Task<HashSet<string>?> GetSubCodesAsync(string code)
        {
            ...
            string query = "SELECT code FROM administrative_areal_2D WHERE code LIKE @code;";
                ...
                if (administrativeAreal2Ds_Current != null && administrativeAreal2Ds_Current.Count != 0 && i != 0)
                {
                    ...
                    if (administrativeAreal2Ds_Previous != null && administrativeAreal2Ds_Previous.Count != 0)
                    {
                        foreach (AdministrativeAreal2D administrativeAreal2D_Current in administrativeAreal2Ds_Current)
                        {
                            ...
                            List<AdministrativeAreal2D>? administrativeAreal2Ds_Filtered = administrativeAreal2Ds_Previous.FindAll(x => x.BoundingBox2D!.InRange(point2D, tolerance));
                            if (administrativeAreal2Ds_Filtered is null || administrativeAreal2Ds_Filtered.Count == 0)
                            {
                                ...
                                administrativeAreal2Ds_Filtered = administrativeAreal2Ds_Previous.FindAll(x => x.BoundingBox2D!.InRange(point2D, tolerance));
                            }
                            ...
                            List<GIS.Classes.AdministrativeAreal2D?>? administrativeAreal2Ds_GIS_Filtered_Temp = administrativeAreal2Ds_GIS_Filtered.FindAll(x => x?.PolygonalFace2D is PolygonalFace2D polygonalFace2D && polygonalFace2D.InRange(point2D, tolerance));
        ...
        public async Task<HashSet<int>?> RemoveAsync(IEnumerable<int>? ids)
        {
            ...
            foreach (int id in ids)
            {
                ...
                NpgsqlBatchCommand npgsqlBatchCommand = new(@"
                    DELETE FROM administrative_areal_2D
                    WHERE id = @id
                    RETURNING id;");
        ...
        public async Task<HashSet<int>?> UpdateAsync(IEnumerable<AdministrativeAreal2D>? administrativeAreal2Ds)
        {
            ...
            foreach (AdministrativeAreal2D administrativeAreal2D in administrativeAreal2Ds)
            {
                ...
                NpgsqlBatchCommand npgsqlBatchCommand = new($@"
                    INSERT INTO administrative_areal_2D (reference, code, name, type_id, min_x, min_y, max_x, max_y, country_id, voivodeship_id, county_id, municipality_id, object)
        ...
        private static AdministrativeAreal2D Create(NpgsqlDataReader npgsqlDataReader)
        {
        ...
        private static async Task<List<AdministrativeAreal2D>?> ReadAsync(NpgsqlCommand npgsqlCommand)
        {
        ...
        private static async Task<List<AdministrativeAreal2D>> ReadAsync(NpgsqlDataReader npgsqlDataReader)
        {
            List<AdministrativeAreal2D> result = [];
