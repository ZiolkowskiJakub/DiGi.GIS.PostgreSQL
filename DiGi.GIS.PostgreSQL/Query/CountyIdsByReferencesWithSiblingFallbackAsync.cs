...
namespace DiGi.GIS.PostgreSQL
{
    public static partial class Query
    {
        ...
        /// <para>A lookup that could not run - the first pass, the widening, or the second pass - answers <c>null</c> for the whole call: a partial map cannot be told apart from a complete one over fewer parts. Degenerate input (no references, no candidate rows) answers an empty map, decided before the widening so that nothing being asked is never classified as a failure.</para>
        ...
        public static async Task<Dictionary<string, int>?> CountyIdsByReferencesWithSiblingFallbackAsync(this Building2DPostgreSQLConverter? building2DPostgreSQLConverter, AdministrativeAreal2DPostgreSQLConverter? administrativeAreal2DPostgreSQLConverter, IEnumerable<string?>? references, IEnumerable<int>? countyIds, Enums.AdministrativeArealType administrativeArealType = Enums.AdministrativeArealType.County, int commandTimeout = 30, CancellationToken cancellationToken = default)
        {
            ...
            // Nothing was asked: decided here, before the widening, so that an absent or empty candidate
            // set is a legitimate empty answer and never classified as a widening that could not run.
            if (references is null || countyIds is null || !countyIds.Any())
            {
                return result;
            }
            ...
            {
                // The widening is part of this method's contract ("before giving up"), so a converter that
                // cannot run it is a lookup that could not run, not a smaller answer.
                return null;
            }
